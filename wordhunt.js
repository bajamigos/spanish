...
/* ==========================
   ...
   ========================== */
function startGame() {
  if (words.length === 0) return alert('Aún no se han cargado las palabras.');
...
function spawnNextWord() {
  ...
  const containerRect = document.querySelector('.game-wrapper').getBoundingClientRect();
  ...
  if (leftZone) {
    ...
    xPos = rightStart + Math.random() * (containerRect.width - rightStart - 50);
  }
